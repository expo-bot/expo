...
begin
  ...
  if ENV['EX_UPDATES_NATIVE_DEBUG'] != '1'
    ...
    resolved_path = `node --print "require.resolve('#{dev_client_package}/package.json', { paths: ['#{__dir__}', '#{project_root}'] })" 2>/dev/null`
    use_dev_client = $?.success? && !resolved_path.strip.empty?
  end
rescue
  use_dev_client = false
end
